...
fn production(prod: u8) -> String {
    match prod {
        // {{{ Productions
        0 => "program -> class_prod ';' program".to_string(),
        1 => "program -> class_prod ';'".to_string(),
        2 => "class_prod -> 'class' 'TYPE' '{' feature_list '}'".to_string(),
        3 => "class_prod -> 'class' 'TYPE' 'inherits' 'TYPE' '{' feature_list '}'".to_string(),
        4 => "class_prod -> 'class' 'TYPE' 'inherits' 'TYPE' '{' '}'".to_string(),
        5 => "feature_list -> feature ';' feature_list".to_string(),
        6 => "feature_list -> feature ';'".to_string(),
        7 => "feature -> 'ID' '(' formal_list ')' ':' 'TYPE' '{' expr '}'".to_string(),
        8 => "feature -> 'ID' '(' ')' ':' 'TYPE' '{' expr '}'".to_string(),
        9 => "feature -> 'ID' ':' 'TYPE' '<-' expr".to_string(),
        10 => "feature -> 'ID' ':' 'TYPE'".to_string(),
        11 => "formal_list -> formal ',' formal_list".to_string(),
        12 => "formal_list -> formal".to_string(),
        13 => "formal -> 'ID' ':' 'TYPE'".to_string(),
        14 => "expr_list -> expr ',' expr_list".to_string(),
        15 => "expr_list -> expr".to_string(),
        16 => "block_list -> expr ';' block_list".to_string(),
        17 => "block_list -> expr ';'".to_string(),
        18 => "assign_list -> 'ID' ':' 'TYPE' '<-' expr ',' assign_list".to_string(),
        19 => "assign_list -> 'ID' ':' 'TYPE' ',' assign_list".to_string(),
        20 => "assign_list -> 'ID' ':' 'TYPE' '<-' expr".to_string(),
        21 => "assign_list -> 'ID' ':' 'TYPE'".to_string(),
        22 => "case_list -> 'ID' ':' 'TYPE' '=>' expr ';' case_list".to_string(),
        23 => "case_list -> 'ID' ':' 'TYPE' '=>' expr ';'".to_string(),
        24 => "expr -> 'ID' '<-' expr".to_string(),
        25 => "expr -> expr '.' 'ID' '(' expr_list ')'".to_string(),
        26 => "expr -> expr '.' 'ID' '(' ')'".to_string(),
        27 => "expr -> expr '@' 'TYPE' '.' 'ID' '(' expr_list ')'".to_string(),
        28 => "expr -> expr '@' 'TYPE' '.' 'ID' '(' ')'".to_string(),
        29 => "expr -> 'ID' '(' expr_list ')'".to_string(),
        30 => "expr -> 'ID' '(' ')'".to_string(),
        31 => "expr -> 'if' expr 'then' expr 'else' expr 'fi'".to_string(),
        32 => "expr -> 'while' expr 'loop' expr 'pool'".to_string(),
        33 => "expr -> '{' block_list '}'".to_string(),
        34 => "expr -> 'let' assign_list 'in' expr".to_string(),
        35 => "expr -> 'case' expr 'of' case_list 'esac'".to_string(),
        36 => "expr -> 'new' 'TYPE'".to_string(),
        37 => "expr -> 'isvoid' expr".to_string(),
        38 => "expr -> expr '+' expr".to_string(),
        39 => "expr -> expr '-' expr".to_string(),
        40 => "expr -> expr '*' expr".to_string(),
        41 => "expr -> expr '/' expr".to_string(),
        42 => "expr -> '~' expr".to_string(),
        43 => "expr -> expr '<' expr".to_string(),
        44 => "expr -> expr '<-' expr".to_string(),
        45 => "expr -> expr '<=' expr".to_string(),
        46 => "expr -> expr '=' expr".to_string(),
        47 => "expr -> 'not' expr".to_string(),
        48 => "expr -> '(' expr ')'".to_string(),
        49 => "expr -> 'ID'".to_string(),
        50 => "expr -> 'integer'".to_string(),
        51 => "expr -> 'string'".to_string(),
        52 => "expr -> 'true'".to_string(),
        53 => "expr -> 'false'".to_string(),
        _ => unreachable!(), 
        // }}}
    }
}

fn head_of_production(prod: u8) -> u8 {
    match prod {
        0 | 1 => 0,
        2..=4 => 1,
        5 | 6 => 2,
...
fn goto(state: u8, nonterminal: u8) -> u8 {
